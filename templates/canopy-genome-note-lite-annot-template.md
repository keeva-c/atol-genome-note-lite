# **A genome assembly for {{ "the " ~ organism.taxonomy_info.ncbi_common_name ~ ", " if organism.taxonomy_info.ncbi_common_name else "" }}*{{ organism.taxonomy_info.ncbi_scientific_name }}*{{ " " ~ organism.taxonomy_info.ncbi_authority if organism.taxonomy_info.ncbi_authority else "" }}**
...
We have assembled a {{ assembly.assembly_level }}-level {% if assembly.haplotypes == 1 %}haploid{% endif %} genome sequence for *{{ organism.taxonomy_info.ncbi_scientific_name }}* ({{ organism.taxonomy_info.ncbi_order }}:{{ organism.taxonomy_info.ncbi_family }}){% if assembly.haplotypes == 2 %}, including two haplotype assemblies. The assembly for haplotype 1 comprises {{ make_pretty_number(assembly.hap_1_scaffold_count) }} scaffolds and spans {{ round_bases_up(assembly.hap_1_genome_length) }}. It has a scaffold N50 of {{ round_bases_up(assembly.hap_1_scaffold_n50) }}, a contig N50 of {{ round_bases_up(assembly.hap_1_contig_n50) }}, and a BUSCO completeness score of {{ assembly.hap_1_busco_c }}%. The assembly for haplotype 2 comprises {{ make_pretty_number(assembly.hap_2_scaffold_count) }} scaffolds and spans {{ round_bases_up(assembly.hap_2_genome_length) }}. It has a scaffold N50 of {{ round_bases_up(assembly.hap_2_scaffold_n50) }}, a contig N50 of {{ round_bases_up(assembly.hap_1_contig_n50) }}, and a BUSCO completeness score of {{ assembly.hap_2_busco_c }}%.{% elif assembly.haplotypes == 1 %}. The assembly comprises {{ make_pretty_number(assembly.hap_1_scaffold_count) }} scaffolds and spans {{ round_bases_up(assembly.hap_1_genome_length) }}. It has a scaffold N50 of {{ round_bases_up(assembly.hap_1_scaffold_n50) }}, a contig N50 of {{ round_bases_up(assembly.hap_1_contig_n50) }}, and a BUSCO completeness score of {{ assembly.hap_1_busco_c }}%.{% else %}. The assembly comprises {% if assembly.assembly_level=='scaffold' %}{{ make_pretty_number(assembly.scaffold_count) }} scaffolds{% elif assembly.assembly_level=='contig' %}{{ make_pretty_number(assembly.contig_count) }} contigs{% endif %} and spans {{ round_bases_up(assembly.genome_length) }}. It has a {% if assembly.assembly_level=='scaffold' %}scaffold N50 of {{ round_bases_up(assembly.scaffold_n50) }}, a {% endif %}contig N50 of {{ round_bases_up(assembly.contig_n50) }} and a BUSCO completeness score of {{ assembly.busco_c }}%.{% endif %} A total of {{ make_pretty_number(annotation.gene_count) }} genes were identified in annotation.

## **Introduction**

### **Species taxonomy**

{{ organism.taxonomy_info.ncbi_full_lineage }}; *{{ organism.taxonomy_info.ncbi_scientific_name }}*{{ " " ~ organism.taxonomy_info.ncbi_authority if organism.taxonomy_info.ncbi_authority else "" }} (NCBI:txid{{ organism.taxonomy_info.ncbi_taxon_id }}).

### **Background**

The genome of {{ "the " ~ organism.taxonomy_info.ncbi_common_name ~ ", " if organism.taxonomy_info.ncbi_common_name else "" }}*{{ organism.taxonomy_info.ncbi_scientific_name }}* was
...
Information relating to sample collection, nucleic acid extraction and library preparation, and sequencing are provided in Tables 1, 2, and 3 respectively.{% if assembly.genomescope_image_path %} A frequency distribution graph of *k*-mers generated during sequencing is included in Figure 1.{% endif %} An overview of the computational pipelines used in genome assembly, annotation, and quality assessment are given in Table 4. Configuration and workflow files used to execute the pipelines are provided in a code repository at [https://github.com/AToL-Bioinformatics/{{ assembly.tol_id }}.{{ assembly.version }}](https://github.com/AToL-Bioinformatics/{{ assembly.tol_id }}.{{ assembly.version }}).
...
| Sample-level BioSample accession | {{ sample.biosample_accession }} |
| Nucleic acid extraction method | {{ experiment.extraction_method }} |
| Nucleic acid treatment | {{ experiment.nucleic_acid_treatment }} |
| Extract concentration (ng/ul) | {{ experiment.nucleic_acid_conc }} |
...
Details about the assembled genome sequence, including key assembly metrics and target minimum standards set by the Earth BioGenome Project (EBP) (Earth BioGenome Project, 2026), are summarised in Table 5. Metrics were calculated immediately after genome assembly. {% if assembly.contaminants_removed %}Following decontamination, {{ assembly.contaminants_removed }} {% if assembly.contaminants_removed == 1 %}contig sequence was{% else %}contig sequences were{% endif %} subsequently removed from the assembly. {% elif assembly.haplotypes == 1 and assembly.hap_1_contaminants_removed %}Following decontamination, {{ assembly.hap_1_contaminants_removed }} {% if assembly.hap_1_contaminants_removed == 1 %}scaffold sequence was{% else %}scaffold sequences were{% endif %} subsequently removed from the assembly. {% elif assembly.haplotypes == 2 and assembly.hap_2_contaminants_removed %}Following decontamination, {{ assembly.hap_1_contaminants_removed }} {% if assembly.hap_1_contaminants_removed == 1 %}scaffold sequence was{% else %}scaffold sequences were{% endif %} subsequently removed from the assembly for haplotype 1 and {{ assembly.hap_2_contaminants_removed }} {% if assembly.hap_2_contaminants_removed == 1 %}scaffold sequence was{% else %}scaffold sequences were{% endif %} subsequently removed from the assembly in haplotype 2.{% endif %}{% if assembly.contact_map_image_path or (assembly.haplotypes == 1 and assembly.hap_1_contact_map_image_path) %} A Hi-C contact map for the assembly is provided in {% if assembly.genomescope_image_path %}Figure 2.{% else %} Figure 1.{% endif %}{% endif %}{% if assembly.hap_1_contact_map_image_path and assembly.hap_2_contact_map_image_path %} Hi-C contact maps for haplotypes 1 and 2 are provided in {% if assembly.genomescope_image_path %}Figures 2 and 3, respectively.{% else %} Figures 1 and 2, respectively.{% endif %}{% endif %} Genome annotation results are provided in Table 6.

{% if assembly.haplotypes == 1 or assembly.haplotypes == 2 %}| | | *EBP standard* |
| --- | ---- | -- |
| **Haplotype 1** | |
...
| Sequencing depth | {{ assembly.coverage }}x | |
...
| BUSCO Reference set | {{ assembly.hap_1_busco_ref }} | |{% endif %}{% if assembly.haplotypes == 2 %}
| **Haplotype 2** | |
...
| BUSCO Reference set | {{ assembly.hap_2_busco_ref }} | |{% endif %}{% if assembly.haplotypes == 1 or assembly.haplotypes == 2 %}
| **Organelles** | |{% if assembly.mito_size %}
| Mitochondrial genome size | {{ make_pretty_number(assembly.mito_size) }} bp | Complete single alleles |{% endif %}{% if assembly.plastid_size %} 
| Plastid genome size | {{ assembly.plastid_size }} bp | Complete single alleles |{% endif %}{% if not assembly.mito_size and not assembly.plastid_size %}
| No organelles assembled | | Complete single alleles |{% endif %}
Table: Table 5: Genome assembly information for {{ assembly.assembly_name|default("*genome assembly name*",true) }}, sequenced from *{{ organism.taxonomy_info.ncbi_scientific_name }}*.{% endif %}{% if assembly.haplotypes == 0 %}| | | *EBP standard* |
| --- | ---- | -- |
...
Table: Table 5: Genome assembly information for {{ assembly.assembly_name|default("*genome assembly name*",true) }}, sequenced from *{{ organism.taxonomy_info.ncbi_scientific_name }}*.{% endif %}
...
project.project_accession }}](https://identifiers.org/ena.embl/{{
...
A code repository containing the workflow configuration files used to generate the genome sequence is hosted on GitHub at [https://github.com/AToL-Bioinformatics/{{ assembly.tol_id }}.{{ assembly.version }}](https://github.com/AToL-Bioinformatics/{{ assembly.tol_id }}.{{ assembly.version }}). Original bioinformatics pipelines are available from the links provided in Table 4.
...
Earth BioGenome Project (2026) Report on Assembly Standards Version 7.0 - January 2026. Retrieved 1 September, 2026, from [https://www.earthbiogenome.org/report-on-assembly-standards](https://www.earthbiogenome.org/report-on-assembly-standards){% if assembly.assembly_level!='contig' %}
