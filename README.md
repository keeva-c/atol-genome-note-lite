 ...
 * a markdown report with key information about how the genome sequence was generated using descriptive text and tabular metadata and assembly metrics
 ...
 ## Full usage
 ```
usage: explore.py [-h] [--wgs_metadata WGS_METADATA [WGS_METADATA ...]] [--hic_metadata [HIC_METADATA ...]] [--rna_metadata [RNA_METADATA ...]] [--output OUTPUT] [--w_annotation] [--wo_annotation]
...
The [`combined_parser.py`](https://github.com/keeva-c/atol-genome-note-lite/blob/main/src/atol_genome_note_lite/combined_parser.py) script can be used to parse ouput files genreated during the assembly and QC processes to extract and format the metrics and information required to generate the genome note lite. Assembly output files which can be parsed include:
 ...
 - contact maps, 
 - GenomeScope2 kmer plots, and
 - mitogenome stats.

### Full usage:

```
usage: combined_parser.py [-h] [--busco BUSCO [BUSCO ...]] [--kmer KMER] [--qv QV] [--summary SUMMARY [SUMMARY ...]] [--assembly_software ASSEMBLY_SOFTWARE]
                          [--map [MAP ...]] [--kmer_plot [KMER_PLOT]] [--mito [MITO]] [--ascc_software ASCC_SOFTWARE] [--treeval_software [TREEVAL_SOFTWARE]]
                          [--metadata [METADATA]] [--output OUTPUT] [--full_json FULL_JSON] [--phased]

This tool parses files that are generated during the genome assembly pipeline and outputs them in a json format suitable for appending to an organism-sample-
experiment-reads metadata json which can be used an input to the explore.py script

options:
  -h, --help            show this help message and exit
  --phased              select this option to extract metrics for phased assemblies. Metrics will be extracted for both haplotypes if sufficient data is proivided.
                        (default: False)

Input:
  --busco BUSCO [BUSCO ...]
                        the JSON summary file generated during BUSCO analysis of the assembly - the file should end with busco.json. Include two summaries for phased
                        haplotypes. (default: None)
  ...
  --summary SUMMARY [SUMMARY ...]
                        the summary text file generated when using the sanger_tol assembly pipeline - the file should end with .assembly_summary. Include two summaries
                        for phased haplotypes. (default: None)
  --assembly_software ASSEMBLY_SOFTWARE
                        the YAML file summarising the tools and versions used by the assembly pipeline - the file should end with genomeassembly_software_versions.yml
                        (default: None)
  --map [MAP ...]       the PreText Snapshot PNG file of the contact map generated during assembly scaffolding - the file should end with FullMap.png. Include two
                        files for phased haplotypes. (default: None)
  --kmer_plot [KMER_PLOT]
                        the GenomeScope2.0 PNG file containing a linear frequency distribution graph of kmers - the file should end with linear_plot.png (default:
                        None)
  ...
  --ascc_software ASCC_SOFTWARE
                        the YAML file summarising tools and versions used by the ASCC decontamination pipeline - the file should end with ascc_software_versions.yml
                        (default: None)
  --treeval_software [TREEVAL_SOFTWARE]
                        the YAML file summarising tools and versions used by the treeval pipeline - the file should end with treeval_software_versions.yml (default:
                        None)
  --metadata [METADATA]
                        the sample metadata JSON to which the assembly stats and metrics will be appended onto - should already include metadata for organism, sample,
                        experiment, and runs (default: None)
...
Output:
  ...
  --full_json FULL_JSON
                        the full JSON metadata object, including metadata for organism, sample, experiment, and runs, and assembly metrics (only generated if a
                        metadata file is included as input) (default: results/full_metadata.json)
```
